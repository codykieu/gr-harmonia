  ...
  namespace harmonia
  {
    ...
    clockbias_phase_est_impl::clockbias_phase_est_impl(int num_platforms,
        ...
        : gr::block("clockbias_phase_est",
          ...
          SNR(SNR),
          time_matrix(num_platforms, std::vector<double>(num_platforms, 0.0f)),
          phase_matrix(num_platforms, std::vector<double>(num_platforms, 0.0f)),
          ...
          check_phase(num_platforms, false)
    {
      ...
      message_port_register_out(PMT_HARMONIA_T_OUT);
    ...
    af::array wrapToPi_af(const af::array &angle)
    {
      ...
      af::array flo = af::floor(tmp);

      return angle - (twoπ * flo);
    }
    ...
    void clockbias_phase_est_impl::handle_msg(pmt::pmt_t msg)
    {
      ...
      for (int tx = 0; tx < num_platforms; ++tx)
      {
        ...
        if (pmt::dict_has_key(d_meta, sdr_syms[tx]))
        {
          ...
          if (pmt::is_f32vector(v))
          {
            auto tmp = pmt::f32vector_elements(v);
            if (!tmp.empty())
      ...
      // TOF and PHI Estimates
      af::array TOF_est = (m_est + m_est.T()) / 2.0;
      af::array PHI_est = (m_est - m_est.T()) / 2.0;
      af::array R_est = TOF_est * 299792458;
      af_print(R_est);
      af_print(TOF_est);
      af_print(PHI_est);

      // Clock Bias Estimates
      af::array cb_est = af::sum(PHI_est, 1) / num_platforms;
      af_print(cb_est);

      int N = num_platforms;
      std::vector<std::vector<double>> A_rows;
      std::vector<double> b_phi;
      A_rows.reserve(N * (N - 1) / 2 + 1);
      b_phi.reserve(N * (N - 1) / 2 + 1);
      ...
      for (int i = 0; i < N; ++i)
      {
        for (int j = i + 1; j < N; ++j)
        {
          ...
          double phi_ij = PHI_est(i, j).scalar<double>();
          b_phi.push_back(phi_ij);
        }
      }
      ...
      // Assumption phi_1 = 0
      {
        ...
        ref[0] = 1.0;
        A_rows.push_back(ref);
        b_phi.push_back(0.0);
      }

      // Flatten A_rows for AF
      int M = A_rows.size();
      std::vector<double> flatA(M * N);
      for (int r = 0; r < M; ++r)
        for (int c = 0; c < N; ++c)
          flatA[r + M * c] = A_rows[r][c];

      // Build AF arrays
      af::array A_phi = af::array(M, N, flatA.data(), afHost);
      af::array b_vec = af::array(M, b_phi.data(), afHost);

      af_print(A_phi);
      // af_print(b_vec);

      // Clock Bias Estimates
      af::array x_phi = af::solve(A_phi, b_vec);
      af_print(x_phi);

      ///////////////////// PRINT /////////////////////////
      std::vector<double> host_phi(3);
      x_phi.host(host_phi.data());
      std::cout << std::fixed << std::setprecision(12)
                << "Clock bias estimates: [";
      for (int i = 0; i < N; ++i)
      {
      ...
      // af::array A_gamma = af::constant(0.0f, base_rows + 1, ntrans);
      // int ind = 0;
      ...
      //       continue;
      //     A_gamma(ind, jj) = +1.0f;
      //     A_gamma(ind, num_platforms + ii) = -1.0f;
      //     ind++;
      //   }
      // }
      // // Gamma_TX = 1 Assumption
      // A_gamma(base_rows, 0) = +1.0f;
      ...
      //     0,
      //     af::constant(var_f, 2 * num_platforms, f64),
      ...
      // af::array x_gamma_est = wrapToPi_af(x_gamma);
      // if (send)
      //   af_print(x_gamma_est);

      std::vector<double> x_host(x_phi.elements());
      x_phi.host(x_host.data());
      ...
      message_port_pub(PMT_HARMONIA_T_OUT, meta);
    }
